...
This docker image is available as [a trusted build on the docker index](https://hub.docker.com/r/lamngockhuong/json-server),
...
```bash
$ docker run -d -p 80:80 -v /home/user/articles.json:/data/db.json lamngockhuong/json-server
```
...
* `-p {OutsidePort}:80` will bind the web server to the given outside port
...
```bash
$ docker run -it --rm lamngockhuong/json-server --help
```
